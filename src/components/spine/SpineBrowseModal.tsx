...
import { EpisodeStoryGrid } from "./EpisodeStoryGrid";

const TITLES: Record<SpineNodeId, string> = {
  episode: "Episode stories",
      ...
      <div className="relative max-h-[55vh] min-h-[240px] overflow-y-auto pr-1">
        ...
        {node === "episode" && (
          <EpisodeStoryGrid
            ...
            onAction={onAction}
            onNewEpisode={() => {
              if (onOpenWizardEpisode) onOpenWizardEpisode();
              else onOpenWizardProcess();
            }}
          />
        )}
        {node === "journey" && (
...
function JourneyBrowse({
  ...
  const existing = (workspace?.journeyCandidates ?? []).filter(
    (c) => c.source === "applied" || c.id.startsWith("existing-")
  );
