...
You MUST respond with valid JSON only. Include source citations grounded in the frameworks above.`,
...

  {
    ...
    description:
      "Deep research on UAE GPSSA social insurance and pension products organised using the Core / Complementary / Non-Core tiering framework, grounded in UAE Federal Pension Law No. 7 of 1999 (and 2023 amendments) and DEWS scheme.",
    targetScreen: "products-portfolio",
    researchType: "products-portfolio",
    systemPrompt: "USE_CANONICAL_PROMPT",
    userPromptTemplate: `Research the following UAE social insurance / pension products: {ITEMS}`,
    model: "gpt-4o",
    maxTokens: 12288,
    temperature: 0.3,
    sortOrder: 5,
  ...
  },
  {
    ...
    description:
      "Coverage matrix research for the UAE labor-market spine (Emirati / Expat / GCC mobile / Military) versus the eight social-protection pillars. Uses canonical Emirati segment vocabulary — never Saudi labels.",
    targetScreen: "products-segments",
    researchType: "products-segments",
    systemPrompt: "USE_CANONICAL_PROMPT",
    userPromptTemplate: `Research UAE coverage levels for the following segment x coverage-type pairs: {ITEMS}`,
    model: "gpt-4o",
    maxTokens: 12288,
    temperature: 0.2,
    sortOrder: 50,
  ...
  },
  {
    ...
    description:
      "Identifies emerging UAE GPSSA product opportunities (digital wallets, gig coverage, voluntary DC top-ups, micro-pensions, parametric insurance) with quantified impact and feasibility scores grounded in international peer innovations.",
    targetScreen: "products-innovation",
    researchType: "products-innovation",
    systemPrompt: "USE_CANONICAL_PROMPT",
    userPromptTemplate: `Research the following UAE GPSSA product-innovation themes: {ITEMS}`,
    model: "gpt-4o",
    maxTokens: 8192,
    temperature: 0.4,
    sortOrder: 6,
...
You MUST respond with valid JSON only. Include source citations.`,
  ...
  {
    id: "intl-delivery-channels",
    name: "International Delivery Channels Research Agent",
    description:
      "Researches the principal social-security institution's customer-facing delivery channels (portal, mobile, branches, call centre, partners, APIs) for all 193 UN countries, with maturity scoring and benchmark comparison.",
    targetScreen: "intl-delivery-channels",
    researchType: "intl-delivery-channels",
    systemPrompt: "USE_CANONICAL_PROMPT",
    userPromptTemplate: `Research delivery channels for the following countries: {ITEMS}`,
    model: "gpt-4o",
    maxTokens: 12288,
    temperature: 0.2,
    sortOrder: 58,
  ...
  {
    id: "intl-delivery-personas",
    name: "International Customer Personas Research Agent",
    description:
      "Produces 4-6 country-representative customer personas grounded in each country's labor-market structure and the principal social-security institution's mandate, for all 193 UN countries.",
    targetScreen: "intl-delivery-personas",
    researchType: "intl-delivery-personas",
    systemPrompt: "USE_CANONICAL_PROMPT",
    userPromptTemplate: `Produce customer personas for the following countries: {ITEMS}`,
    model: "gpt-4o",
    maxTokens: 12288,
    temperature: 0.3,
    sortOrder: 59,
  ...
  {
    id: "intl-delivery-models",
    name: "International Delivery Models Research Agent",
    description:
      "Captures each country's principal social-security institution operating / delivery models (digital-first, employer-mediated, partner-bank, embedded-government, agent network, etc.) with maturity, enablers, risks, and peer benchmarks.",
    targetScreen: "intl-delivery-models",
    researchType: "intl-delivery-models",
    systemPrompt: "USE_CANONICAL_PROMPT",
    userPromptTemplate: `Research delivery models for the following countries: {ITEMS}`,
    model: "gpt-4o",
    maxTokens: 12288,
    temperature: 0.2,
    sortOrder: 60,
